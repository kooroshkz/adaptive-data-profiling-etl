...
\noindent
\textbf{Aim.} This thesis investigates whether machine learning can automatically learn what normal data looks like and reliably flag deviations, serving as a low-cost complement to rule-based checks at the point where data first enters an ETL pipeline.

\noindent
\textbf{Method.} We built and deployed a complete ETL pipeline processing hourly weather and electricity data, with artificial anomalies injected at known positions to measure detection accuracy and computational cost. We evaluated two automated approaches, both driven by the Optuna optimisation framework: classical machine learning model selection (AutoML), which searches over the PyOD anomaly detectors, and Neural Hyperparameter Optimization (Neural HPO), which searches over PyTorch autoencoders.
...
\noindent
\textbf{Conclusion.} AutoML-based anomaly detection is a practical complement to rule-based checks in ETL pipelines. Its effectiveness and cost depend strongly on the nature of the incoming data, making it well-suited for columns with stable, predictable distributions and less effective for noisy or irregular ones.
\end{abstract}
...
Anomaly detection itself is a well-researched field, with a large body of work on both classical and deep methods \cite{chandola2009anomaly, han2022adbench}. The contribution of this thesis is therefore not a new detection algorithm but where the detection runs and how it is operated: we place per-column unsupervised detection at the ingestion stage of an ETL pipeline, before any transformation or loading, and we automate model selection so that no manual rule-writing or specialist tuning is needed. To make the approach reusable beyond the experiments reported here, we built it into an open-source Python library, the Adaptive Profiler, which we released on PyPI and GitHub \cite{repoadaptiveprofiler2026}.
...
This section introduces the concepts the rest of the thesis builds on: how data pipelines and ETL processes work in practice, why data quality is hard to guarantee, the case for validating data early, and the unsupervised anomaly detection and AutoML techniques used in the system.
...
This section positions the thesis against prior work on machine-learning-based data quality, the limitations of existing rule-based tools, AutoML for anomaly detection, and the comparison between classical and neural detectors on tabular data.
...
This work follows a build-and-evaluate approach: this section describes the system we built, and Section~\ref{sec:expsetup} describes how we evaluated it. We designed and deployed a complete ETL pipeline on AWS, using Apache Airflow for orchestration, dbt for SQL transformations, and DuckDB as a data warehouse. We developed a custom Python library, the Adaptive Profiler, and integrated it at the ingestion stage. It adds two quality check layers on each incoming batch: a rule-based layer for data contract checks, and an ML-based layer that trains an anomaly detection model per column using PyOD and Optuna. Both layers are configured through a single YAML file.
...
To evaluate the system, we ingested hourly weather data from five cities from the Open-Meteo API and injected controlled anomalies as ground-truth labels. We ran three experiments, one per research sub-question: detection effectiveness, computational overhead and scaling, and a comparison between classical AutoML and neural detector search.
...
This experiment measures how accurately ML-based detection identifies the injected anomalies, reported first in detail for Amsterdam, then across all five cities, and finally on the electricity dataset to test generalization.
...
This section interprets the results of the three experiments: what determines detection effectiveness, why classical detectors outperform neural search, how the decision threshold trades off false positives against recall, and what the findings mean for training on large datasets.
...
This section summarises the answers to the three research sub-questions and outlines directions for further work.
