...
One main motivation is the principle of \emph{shift-left} data quality: validating data as early as possible in the pipeline, ideally at the point where it first enters \cite{datakitchen2023shiftleft}. The same idea has long been part of software engineering practice, where defects are generally cheaper to address the earlier in the lifecycle they are caught. If a bad record enters the pipeline, every transformation that follows inherits the defect. Worse, once a transformation aggregates or summarizes the data, the bad value is permanently baked into the stored result and can no longer be removed downstream, and by the time it reaches an aggregated report or a trained model it can be impossible to trace which source record caused the problem. Recent work argues that \emph{when} validation happens, not only which method is used, determines both the final error rate and the cost of correction: Kothari et al.\ \cite{kothari2026earlyqa} model validation timing as a measurable design variable and show that catching errors before they propagate is substantially cheaper than catching them after downstream stages have consumed them, while Karlas et al.\ \cite{karlas2025navigating} show that data errors compound as they move through the stages of a machine learning pipeline, so an error's impact depends strongly on where it is caught. This is consistent with the broader data-centric view that data quality should be treated as a first-class concern across the pipeline rather than patched downstream \cite{whang2023datacentric}.
...
We selected PyOD \cite{zhao2019pyod}, an open-source Python library that implements a wide range of outlier-detection algorithms behind a single API, over the broader AutoML libraries H2O AutoML, AutoGluon, and PyCaret for its targeted unsupervised detection models and simple integration into the Adaptive Profiling library.

Four algorithms from PyOD were selected by Optuna, a hyperparameter optimization framework that automatically searches for the best-performing model and configuration (detailed in Section~\ref{sec:automl}), across all trained models in this project:
...
\subsection{AutoML and Hyperparameter Optimization}\label{sec:automl}
...
\begin{itemize}
  \item AWS EC2 (a Linux server) as the compute host, running the pipeline inside Docker containers
  \item Apache Airflow for pipeline orchestration and scheduling
  \item External weather APIs as the data source for the Extract stage
  \item dbt for data transformation and schema metadata
  \item Amazon S3 as the data lake where processed data is loaded
  \item DuckDB/MotherDuck as a SQL warehouse for local analytics and cloud sharing
  \item A Next.js dashboard for visualizing the results
  \item GitHub Actions for continuous integration and deployment
  \item A custom Python library for ML-based anomaly detection using PyOD and Optuna \cite{repoadaptiveprofiler2026}
\end{itemize}

Figure~\ref{fig:workflow} shows how these tools fit together. The pipeline runs on an Amazon EC2 Linux server, where Apache Airflow executes inside Docker containers. Airflow orchestrates the three ETL stages: it extracts data from external weather APIs (Extract), dbt transforms it (Transform), and the results are loaded into an Amazon S3 data lake (Load). From S3, DuckDB/MotherDuck provides SQL analytics and a Next.js dashboard provides visualization, while EC2 reads from and writes to S3. GitHub Actions automates continuous integration and deployment. In the figure, solid arrows show data and control flow, dashed arrows show automation and CI flow, and dashed boxes mark external sources. The AutoML anomaly detection that this thesis adds is integrated into the Airflow workflow and is shown in detail in the pipeline DAG of Figure~\ref{fig:intended_etl}.

\begin{figure}[t]
  \begin{center}
  \includegraphics[width=0.95\textwidth]{images/workflow_diagram_new}
  \end{center}
  \caption{System architecture and tool integration. Apache Airflow runs in Docker on an Amazon EC2 server (on AWS) and orchestrates the Extract (external weather APIs), Transform (dbt), and Load (Amazon S3 data lake) stages. From S3, DuckDB/MotherDuck serves SQL analytics and a Next.js dashboard provides visualization; GitHub Actions handles CI/CD. Solid arrows are data and control flow, dashed arrows are automation and CI flow, and dashed boxes mark external sources.}
  \label{fig:workflow}
\end{figure}
...
This experiment measures how accurately ML-based detection finds injected anomalies compared to rule-based checks. We first describe the injected anomalies, then report detection results for Amsterdam, the algorithm Optuna selected across all cities, the multi-city comparison, the effect of shift magnitude, and finally a generalization test on electricity demand data.
...
\begin{axis}[
  ...
  nodes near coords,
  nodes near coords style={font=\small, /pgf/number format/.cd, fixed, precision=0},
  legend style={at={(0.5,1.03)}, anchor=south, font=\small, legend columns=2,
...
\subsubsection{Sensitivity to Anomaly Rate and Shift Magnitude}
\label{sec:elec_sensitivity}

The electricity results so far fix a single anomaly rate ($\approx$2.5\%) and use large 50--100\% shifts. To map \emph{where} AutoML detection succeeds or fails on this domain, we sweep the two factors that govern difficulty: the \emph{anomaly rate} (the fraction of records made anomalous) and the \emph{shift magnitude} (how far each injected value is moved, in either direction). For every combination we inject fresh anomalies into the raw demand series and run the identical AutoML search used throughout this work (Optuna over the same PyOD detectors, F2 objective, seasonality-adjusted contextual features), changing nothing about the detector itself. We report the \emph{capture rate}: the share of the injected anomalies the model flags, that is, recall against the known ground truth, the same quantity Figure~\ref{fig:elec_shift} reports for one setting. The grid covers anomaly rates of $\{1, 2, 5, 10, 20\}\%$ and shift magnitudes of $\{10, 20, 30, 40, 50\}\%$ with random sign; each cell is the mean of three injection seeds (the across-seed variation is small), and results are reported separately for the two demand columns, INDO and ITSDO. Table~\ref{tab:elec_sensitivity} gives the capture rates and Figure~\ref{fig:elec_sensitivity_surf} shows the same values as a surface.
...
\textbf{INDO}\par\vspace{3pt}
\begin{tabular}{@{}l ccccc@{}}
\toprule
& \multicolumn{5}{c}{Shift magnitude}\\
\cmidrule(l){2-6}
Rate & 10\% & 20\% & 30\% & 40\% & 50\% \\
\midrule
1\% & \cellcolor[RGB]{254,251,185}49 & \cellcolor[RGB]{201,232,128}63 & \cellcolor[RGB]{117,195,100}78 & \cellcolor[RGB]{69,173,90}\textcolor{white}{84} & \cellcolor[RGB]{54,165,87}\textcolor{white}{86} \\
2\% & \cellcolor[RGB]{227,243,153}57 & \cellcolor[RGB]{119,196,100}77 & \cellcolor[RGB]{87,181,95}82 & \cellcolor[RGB]{60,168,88}\textcolor{white}{85} & \cellcolor[RGB]{24,149,78}\textcolor{white}{90} \\
5\% & \cellcolor[RGB]{187,226,119}66 & \cellcolor[RGB]{75,175,92}\textcolor{white}{83} & \cellcolor[RGB]{21,143,75}\textcolor{white}{92} & \cellcolor[RGB]{23,147,77}\textcolor{white}{91} & \cellcolor[RGB]{16,134,70}\textcolor{white}{94} \\
10\% & \cellcolor[RGB]{218,239,141}60 & \cellcolor[RGB]{72,174,91}\textcolor{white}{84} & \cellcolor[RGB]{24,149,78}\textcolor{white}{91} & \cellcolor[RGB]{39,158,83}\textcolor{white}{88} & \cellcolor[RGB]{18,137,72}\textcolor{white}{93} \\
20\% & \cellcolor[RGB]{254,250,183}49 & \cellcolor[RGB]{191,227,122}65 & \cellcolor[RGB]{139,205,103}74 & \cellcolor[RGB]{114,194,100}78 & \cellcolor[RGB]{93,184,96}81 \\
\bottomrule
\end{tabular}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}\centering
\textbf{ITSDO}\par\vspace{3pt}
\begin{tabular}{@{}l ccccc@{}}
\toprule
& \multicolumn{5}{c}{Shift magnitude}\\
\cmidrule(l){2-6}
Rate & 10\% & 20\% & 30\% & 40\% & 50\% \\
\midrule
1\% & \cellcolor[RGB]{254,250,183}48 & \cellcolor[RGB]{144,207,103}73 & \cellcolor[RGB]{137,204,102}74 & \cellcolor[RGB]{45,161,84}\textcolor{white}{87} & \cellcolor[RGB]{17,136,71}\textcolor{white}{93} \\
2\% & \cellcolor[RGB]{242,249,173}53 & \cellcolor[RGB]{93,184,96}81 & \cellcolor[RGB]{57,167,87}\textcolor{white}{86} & \cellcolor[RGB]{39,158,83}\textcolor{white}{88} & \cellcolor[RGB]{22,145,76}\textcolor{white}{91} \\
5\% & \cellcolor[RGB]{157,213,105}71 & \cellcolor[RGB]{60,168,88}\textcolor{white}{85} & \cellcolor[RGB]{25,151,79}\textcolor{white}{90} & \cellcolor[RGB]{13,128,67}\textcolor{white}{95} & \cellcolor[RGB]{27,152,80}\textcolor{white}{90} \\
10\% & \cellcolor[RGB]{205,233,131}62 & \cellcolor[RGB]{63,170,89}\textcolor{white}{85} & \cellcolor[RGB]{22,145,76}\textcolor{white}{91} & \cellcolor[RGB]{17,136,71}\textcolor{white}{93} & \cellcolor[RGB]{25,151,79}\textcolor{white}{90} \\
20\% & \cellcolor[RGB]{254,248,179}48 & \cellcolor[RGB]{191,227,122}65 & \cellcolor[RGB]{139,205,103}74 & \cellcolor[RGB]{112,193,100}78 & \cellcolor[RGB]{99,187,98}80 \\
\bottomrule
\end{tabular}
\end{minipage}
\caption{Capture rate (\%) of injected anomalies for GB electricity demand, by anomaly rate (rows) and shift magnitude (columns), for the two demand columns. Each value is the recall of the injected ground truth, averaged over three injection seeds. Cells are shaded from red (low capture) to green (high). Detection strengthens with shift magnitude and is strongest at intermediate anomaly rates (5--10\%).}
\label{tab:elec_sensitivity}
\end{table}
...
\begin{groupplot}[
  ...
  view={-60}{22},
  colormap={rdylgn}{rgb255(0cm)=(215,48,39); rgb255(1cm)=(252,141,89); rgb255(2cm)=(254,224,139); rgb255(3cm)=(145,207,96); rgb255(4cm)=(26,152,80)},
  point meta min=0, point meta max=1,
  xlabel={Shift}, ylabel={Rate}, zlabel={Capture rate},
  xtick={1,2,3,4,5}, xticklabels={10\%,20\%,30\%,40\%,50\%},
  ytick={1,2,3,4,5}, yticklabels={1\%,2\%,5\%,10\%,20\%},
  zmin=0, zmax=1, ztick={0,0.25,0.5,0.75,1.0},
  xticklabel style={font=\scriptsize}, yticklabel style={font=\scriptsize},
  zticklabel style={font=\scriptsize},
  xlabel style={font=\small}, ylabel style={font=\small}, zlabel style={font=\small},
  title style={font=\small\bfseries},
]
\nextgroupplot[title={INDO}]
\addplot3[surf, mesh/cols=5, faceted color=black!35] coordinates {
(1,1,0.489) (2,1,0.633) (3,1,0.776) (4,1,0.841) (5,1,0.860)

(1,2,0.571) (2,2,0.772) (3,2,0.817) (4,2,0.853) (5,2,0.904)

(1,3,0.659) (2,3,0.833) (3,3,0.916) (4,3,0.907) (5,3,0.937)

(1,4,0.598) (2,4,0.837) (3,4,0.906) (4,4,0.880) (5,4,0.928)

(1,5,0.486) (2,5,0.652) (3,5,0.739) (4,5,0.780) (5,5,0.810)
};
\nextgroupplot[title={ITSDO}]
\addplot3[surf, mesh/cols=5, faceted color=black!35] coordinates {
(1,1,0.485) (2,1,0.731) (3,1,0.742) (4,1,0.871) (5,1,0.932)

(1,2,0.535) (2,2,0.810) (3,2,0.857) (4,2,0.881) (5,2,0.911)

(1,3,0.713) (2,3,0.853) (3,3,0.901) (4,3,0.946) (5,3,0.897)

(1,4,0.622) (2,4,0.849) (3,4,0.911) (4,4,0.930) (5,4,0.899)

(1,5,0.479) (2,5,0.651) (3,5,0.740) (4,5,0.783) (5,5,0.804)
};
\end{groupplot}
\end{tikzpicture}
\caption{Capture-rate surface over anomaly rate and shift magnitude for the two demand columns (same values as Table~\ref{tab:elec_sensitivity}; height and colour both encode capture rate). The surface rises steeply along the shift axis and ridges at the intermediate 5--10\% anomaly rates, falling off at the 1\% and 20\% extremes.}
\label{fig:elec_sensitivity_surf}
\end{figure}

Two effects stand out, both consistent with the rest of the thesis. First, \textbf{shift magnitude is the dominant driver of detection}. Capture climbs steeply from roughly one half at a 10\% shift to 80--95\% by a 40--50\% shift, the same monotone trend already seen for weather (Figure~\ref{fig:shift}) and for the single-rate electricity run (Figure~\ref{fig:elec_shift}). A 10\% shift on demand is a genuinely hard contextual anomaly: it often stays inside the normal daily range and is unusual only relative to the time of day, so a large fraction slips past. From a 30\% shift onward the model catches the clear majority (74--92\%).

Second, \textbf{the anomaly rate has a sweet spot around 5--10\%}. Detection is weaker at both extremes. At 1\% the unsupervised search has very few positive examples to calibrate its decision threshold against, so it stays conservative; at 20\% the injected anomalies contaminate the very distribution the model treats as normal, pulling the learned ``normal'' band toward them so that it begins to accept them. This is a direct empirical instance of the unsupervised limitation noted in Section~\ref{sec:discussion}, that anomalies already present in the training data are learned as normal. INDO and ITSDO produce almost identical surfaces, as expected since ITSDO is INDO plus station load and interconnector exports, so the two series move together. Across the grid Optuna again favoured the tail- and density-based detectors (ECOD and HBOS) that suit demand's tight, seasonal distribution, the same families it selected in the main electricity run (Table~\ref{tab:elec_before_after}).
...
$\delta$ & Column exponent   & How training time grows with columns & $0$--$1$ \\
...
\end{tikzpicture}
\end{center}
\caption{Fitted row scaling exponent $\beta$ per algorithm from ~90 benchmark observations. Each $\beta$ is fitted separately per detector, and the fitted curves match the measured training times closely (from 92\% of the variation for IForest to over 99\% for ECOD, COPOD, and LOF).}
\label{fig:beta_chart}
\end{figure}

\subsubsection{How Close Is the Projected Time to Reality?}
\label{sec:projection_accuracy}

The scaling formula is only useful if the time it \emph{estimates} is close to the time training actually \emph{takes}. Because the projection tool is meant to be calibrated on a small sample and then extrapolated to the full dataset, its accuracy is best judged as a function of how much data it is given. We measured this on surface pressure, the most predictable column in the detection experiments (Section~\ref{sec:experiments}): the real training time was recorded across a grid of row counts (median of seven warm repeats, using the ECOD detector Optuna selected for this column and $k=25$ trials per column). For each data fraction the law was fit on only the rows up to that fraction and extrapolated to the full $n_{\text{full}}=20{,}352$ rows, then compared against the measured full-scale time. The time reported throughout is the cost of one full per-column search, that is, all $25$ trials. At $25\%$ this is a four-fold extrapolation; at $100\%$ it reduces to interpolation. A small fixed per-fit offset is included in the fit, so the exponent reflects the true data scaling rather than being dragged down by the constant setup cost that dominates the smallest samples.
...
\begin{tabular}{@{}l r r r r r@{}}
\toprule
Data & Rows & $\beta$ & Pred. & Actual & Error \\
\midrule
...
\begin{axis}[
  ...
  xlabel={Data given to the tool (\%)},
  ylabel={Search time, $k=25$ (s)},
  xmin=15, xmax=110, ymin=0.060, ymax=0.080,
  xtick={25,50,75,100},
  ytick={0.060,0.065,0.070,0.075,0.080},
  yticklabel={\pgfmathprintnumber[fixed,precision=3]{\tick}},
  tick align=outside,
  axis line style={draw=gray!60},
  ymajorgrids=true, major grid style={draw=gray!20},
  legend style={at={(0.97,0.97)}, anchor=north east, font=\scriptsize,
                draw=codeborder, fill=codebg},
  legend cell align=left,
]
% measured full-scale time (constant reference)
\addplot[recall!70!black, dashed, line width=1pt] coordinates {(15,0.067) (110,0.067)};
\addlegendentry{Actual (full scale)}
% projected time, with error labels, converging onto the actual line
\addplot[leidenblue, line width=1.1pt, mark=*, mark size=2.2pt,
  nodes near coords, point meta=explicit symbolic,
  every node near coord/.append style={font=\tiny, anchor=south,
    yshift=2pt, color=leidenblue!80!black}]
  coordinates {(25,0.073)[8.9\%] (50,0.069)[3.0\%] (75,0.067)[0.5\%] (100,0.067)[0.5\%]};
\addlegendentry{Projected}
\end{axis}
\end{tikzpicture}
\end{minipage}
\caption{Projected versus measured full-scale training time for surface pressure (ECOD, $n_{\text{full}}=20{,}352$, $k=25$), as the projection tool is calibrated on an increasing fraction of the data. Times are the cost of one full per-column search (all 25 trials). \textbf{Left:} the rows seen, fitted row exponent $\beta$, projected and actual search time, and the resulting error. \textbf{Right:} the projected time (blue, labelled with its error) converging onto the measured full-scale time (green dashed). From three quarters of the data the two are visually indistinguishable.}
\label{tab:projection_pressure}
\end{table}
