...
def suggest_model_and_params(trial: optuna.Trial, input_dim: int) -> tuple[str, dict[str, Any]]:
    ...
    if model_name in ("AE", "VAE"):
        ...
        # Build decreasing hidden dims for encoder (decoder is mirrored in nn_models)
        hidden_dims = []
        w = layer_width
        ...
        params["nu"] = trial.suggest_float("nu", 0.001, 0.20, log=True)
        gamma_mode = trial.suggest_categorical("gamma_mode", ["scale", "float"])
        if gamma_mode == "float":
            params["gamma"] = trial.suggest_float("gamma_val", 0.001, 2.0, log=True)
        else:
            params["gamma"] = gamma_mode  # "scale"
